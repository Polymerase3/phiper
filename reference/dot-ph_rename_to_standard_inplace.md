# Rename columns to PHIPER standard names in-place

`.ph_rename_to_standard_inplace()` renames columns in a DuckDB table or
view to the standard PHIPER schema using a mapping of standard names to
source columns. For views, it recreates the view with aliased columns;
for tables, it issues `ALTER TABLE ... RENAME COLUMN` statements.
...
- colname_map:
  ...
  Named character list mapping **standard** PHIPER column names (e.g.
