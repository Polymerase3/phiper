# Read and register "long" phiper data into a DuckDB-backed database
...
each source column to the standard PHIPER names. The resulting
...
- Column renaming is performed in SQL with `AS`, so no R-level
  [`rename()`](https://dplyr.tidyverse.org/reference/rename.html) calls
  are needed.
