...
- **Enrichment counts** — `plot_enrichment_counts()` gives a quick
  overview of per-group peptide hit counts.
- **Alpha diversity** — richness, Shannon, Simpson, Pielou evenness, and
  ...
  bracket annotations (`compute_alpha()`,
...
- **Beta diversity** — pairwise distances, PCoA, CAP (constrained
  ...
  (`compute_distance()`, `compute_pcoa()`, `compute_capscale()`,
  `compute_permanova()`, `compute_dispersion()`, `compute_tsne()`).
...
data analysis and HTML reports). Both use the built-in `theme_phip()`
